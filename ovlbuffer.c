...
static int ovlbuf_setupfb(struct ovlbuf_dev *fbdev)
{
	...
	*/
	...
	if (!desc) {
		pr_err("Failed to prepare from-buffer DMA descriptor\n");
		return -ENOMEM;
	}
	...
	if (!desc) {
		pr_err("Failed to prepare to-buffer DMA descriptor\n");
		return -ENOMEM;
	}
	dmaengine_submit(desc);
...
static int ovlbuf_probe(struct platform_device *pdev)
{
	...
	if (ret)
		dev_err(&pdev->dev, "Missing hsize property, assume 960\n");
	ret = of_property_read_u32(np, "vsize", &fbdev->hsize);
	if (ret)
		dev_err(&pdev->dev, "Missing vsize property, assume 960\n");
	...
	pr_debug("%s virt=%p phys=%x size=%u\n", __func__,
